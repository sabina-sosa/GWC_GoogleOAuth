...
Follow these steps to get your own copy working. You'll need to set up both Appwrite and Google OAuth first.
 ...
 Part 1: Appwrite Setup
   ...
   ☆Create a New Project☆
      ...
      - You'll need this for `APPWRITE_PROJECT_ID` in `script.js`

   ☆Create a Database☆
      - Go to Databases in the left sidebar
      - Click "Create Database"
      - Give it a name (e.g., "NotesDB")
      - Copy the Database ID you'll need this for `NOTES_DATABASE_ID` in `script.js`
   ...
   ☆Create a Table☆
      ...
      - Copy the Table ID also needed this for `NOTES_COLLECTION_ID` in `script.js`
...
Part 3: Configure Your Appwrite Credentials
   ...
   ☆Where to find these values☆
      ...
      - **NOTES_COLLECTION_ID**: Appwrite Console → Databases → [Your Database] → [Your Tble] → Copy ID from URL or Settings (or use the table name like 'notes')
...
Part 4: Set Up Local Development Server
   ...
   *Important Note*
   ! Appwrite OAuth only works if your app is being served from a real local URL, not just opening the HTML file directly !

Part 5: Test Your Setup
   - The server should open automatically, or go to `http://localhost:___`
